...
namespace CTRPluginFramework
{
	...
	Result MCUHWC_ReadRegister(u8 reg, void* data, u32 size)
	{
		...
		if(R_FAILED(ret = svcSendSyncRequest(mcuHwcHandle)))
			return ret;
	...
	void OnNewFrameCallback(Time ActualTime)
	{
		...
		TopScreen.DrawRect(coordx, coordy1, 340, 24, settings.BackgroundMainColor, true);
		TopScreen.DrawRect(coordx + 2, coordy1 + 2, 340 - 4, 20 - 2, settings.BackgroundBorderColor, false);
		
        time(&rawtime);
        timeinfo = localtime(&rawtime);
        TopScreen.DrawSysfont(Utils::Format("　%04d/%02d/%02d(%s)　%02d:%02dm%02ds　Battery:",
		...
		BottomScreen.DrawSysfont("Luna", coordx + 5, coordy1 + 3); // Top BottomScreen

		BottomScreen.DrawSysfont("Discord:thib.py", coordx + 5, coordy2 + 3); // Bottom BottomScreen*/
	}
}