...
{
    std::unordered_map<int, std::u16string> Render::_randomSets;
    ...
    std::string Render::PullRandomCharacter(int setID)
    {
        ...
        if (it == _randomSets.end())
            return "";
        std::u16string& choices = it->second;
        if (choices.empty())
            return "";
        char16_t newStr[2];
    ...
    void Render::Interface::DrawLinuxString(const std::string& str, int posX, int posY, const Color& foreground, const Color& background) {
        Renderer::DrawString(str.c_str(), posX, posY, foreground, background);
    }
