...
namespace CTRPluginFramework
{
    ...
    void Utils::ConvertUTF8ToUTF16(std::u16string& out, const char* utf8str)
    {
        u32 utf32;
        char16_t utf16[2+1]; // Size + NULL
        ...
        while (*utf8str)
        {
            ...
            if (consumed < 0)
                break;
            utf8str += consumed;
            int created = encode_utf16(reinterpret_cast<uint16_t*>(utf16), utf32);
            if (created < 0)
                break;
            utf16[created] = 0;
            out.append(utf16);
    ...
    void Utils::ConvertUTF16ToUTF8(std::string& out, const char16_t* utf16str)
    {
        u32 utf32;
        char utf8[4+1]; // Size + NULL
        out.reserve(out.size() + std::char_traits<char16_t>::length(utf16str)); // Output size will be approximately the amount of chars of input
        while (*utf16str)
        {
            int consumed = decode_utf16(&utf32, reinterpret_cast<const uint16_t*>(utf16str));
            if (consumed < 0)
                break;
            utf16str += consumed;
